/*
 * ANSI text formatting codes
 *
 * Beware: formats do not reset on their own.
 * Be sure to print DEFCOLOUR before the call
 * of another function or the end of the current
 * function. This will avoid format contamination.
 */

#ifndef LWCT_COLOURS_H
#define LWCT_COLOURS_H

#define DEFCOLOUR "\033[0m"
#define ULINED_DEFCOLOUR "\033[4m"
#define RED "\033[0;91m"
#define BOLD_RED "\033[1;91m"
#define GREEN "\033[0;92m"
#define BOLD_GREEN "\033[1;92m"
#define YELLOW "\033[0;93m"
#define BOLD_YELLOW "\033[01;93m"
#define BLUE "\033[0;94m"
#define BOLD_BLUE "\033[1;94m"
#define MAGENTA "\033[0;95m"
#define BOLD_MAGENTA "\033[1;95m"
#define CYAN "\033[0;96m"
#define BOLD_CYAN "\033[1;96m"
#define WHITE "\033[0;97m"
#define BOLD_WHITE "\033[1;97m"

#endif
