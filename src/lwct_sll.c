...
/*
 * ITERATE Macro: Navigate in list
 */
#define ITERATE(l, p) for (struct lwct_sll_node *p = l->first; p != NULL; p = p->next)
...
lwct_sll_ret lwct_sll_contains(lwct_sll *sll, void *info)
{
	if (!sll || !info)
		return LWCTL_SLL_PARAM;
